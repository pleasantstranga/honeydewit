...
import com.ajbtechnologies.services.DeleteListService;
		...
		checked.setOnClickListener(new OneOffClickListener() {
			...
			public void onClick(View v) {
				changeStrikeThroughText(checked, (LinearLayout)v.getParent());	
				 ((Application)getContext().getApplicationContext()).getShoppingListDbHelper().setListChecked(checked.isChecked(), data.getListId());
			}
		...
		dialog.setPositiveButton(getContext().getText(R.string.delete).toString(), new View.OnClickListener() {
			...
			public void onClick(View view) {
				if (data != null) {
					...
					dialog.dismiss();
					((Application) getContext().getApplicationContext()).getListsIdsToDelete().add(listId);
