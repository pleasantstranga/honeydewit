...
public class BasicList extends BaseObject {
	...
	public void addListEvents(Context context, List<ListEvent> listEvents) {
		if(events == null) {
			try {
				events = ((HoneyDewApplication)context).getShoppingListDbHelper().getListDao().getEmptyForeignCollection(BasicList.EVENTS);
	...
	public void addErrors(Context context, List<ImportError> errors) {
		if(this.errors == null) {
			try {
				this.errors = ((HoneyDewApplication)context).getShoppingListDbHelper().getListDao().getEmptyForeignCollection(BasicList.ERRORS);
	...
	public void addEvent(Context context, ListEvent listEvent) {
		if(events == null) {
			try {
				events = ((HoneyDewApplication)context).getShoppingListDbHelper().getListDao().getEmptyForeignCollection("events");
